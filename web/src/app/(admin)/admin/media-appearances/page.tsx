...
import { getAdminMediaPage, parseAdminMediaFilters } from "@/lib/admin/media-list";
import { requireRole } from "@/lib/auth/require-role";
import { MEDIA_FORMAT_VALUES } from "@/lib/media/filters";
import { routes } from "@/lib/routes";

export default async function MediaAppearancesAdminPage({
  searchParams,
}: {
  ...
  const filters = parseAdminMediaFilters(new URLSearchParams(flatten(params)));
  const page = await getAdminMediaPage(filters);

  const prevHref = page.pagination.hasPreviousPage
    ? toMediaHref({ ...filters, page: filters.page - 1 })
    : null;
  ...
  return (
    <section className="space-y-6">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold">Media Appearances</h1>
        <p className="text-sm text-(--color-muted-fg)">Manage media records, links, formats, and visibility.</p>
        <Link
          href={`${routes.admin.mediaAppearances}/new`}
      ...
      <Card>
        <form className="grid gap-3 md:grid-cols-[1fr_170px_130px_170px_auto]">
          <input
            name="q"
            defaultValue={filters.q ?? ""}
            placeholder="Search title, outlet, summary"
      ...
      <Card className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Media entries</h2>
          <p className="text-sm text-(--color-muted-fg)">{page.pagination.totalRows} total</p>
        </div>
        {page.rows.length === 0 ? (
          <p className="text-sm text-(--color-muted-fg)">No media entries match this filter.</p>
        ) : (
            ...
            {page.rows.map((row) => (
              <li key={row.id} className="flex items-center justify-between gap-3 rounded-md border border-(--color-muted) p-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-(--color-muted-fg)">{row.outlet}</p>
                  <Link
                    href={`${routes.admin.mediaAppearances}/${encodeURIComponent(row.id)}/edit`}
                ...
                <div className="shrink-0 text-right">
                  <p className="text-xs uppercase tracking-wide text-(--color-muted-fg)">{row.format}</p>
      ...
      <div className="flex items-center justify-between text-sm">
        <Link
          href={prevHref ?? routes.admin.mediaAppearances}
        ...
        <Link
          href={nextHref ?? routes.admin.mediaAppearances}
...
function toMediaHref(filters: {
  q?: string;
  format?: "interview" | "podcast" | "article" | "panel" | "video";
  year?: number;
  ...
  return `${routes.admin.mediaAppearances}?${search.toString()}`;
}
